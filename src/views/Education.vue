...
<hr style="margin-top: 50px;">
		...
		<div class="commands">
			...
			<label><input style="margin: 0 5px 0 0" type="button" id ="reset">Reset</label>
		</div>	  
		<div class="commands">
  ...
  <div id = 'pie_' >
	...
	<select id = 'c2'>
		...
		<option value = 'East Asia and the Pacific'>East Asia and the Pacific</option>
		<option value = 'Latin America and the Caribbean'>Latin America and the Caribbean</option>
		<option value = 'North America and Western Europe' selected>North America and Western Europe</option>
		<option value = 'South and West Asia'>South and West Asia</option>
	</select>
  <button v-on:click = "draw_pie" style="margin-left:14px">Show Details</button>
...
d3.select('#dist2').selectAll('*').remove();
   ...
   // .sortValues(d3.descending);  //🎒 explain: 
    ...
    var label = d3.arc()  //🎒 explain: 
       ...
       var arc2 = arc.append('path')  //🎒 explain: 
        //🎒 explain:
           .attr('fill', d => color(d.data.Level))  //🎒 explain: 
           .attr('opacity',0.5)
           .attr('stroke','white')
           arc2.transition().delay(function(d,i){
               return i*200
           }).attrTween('d', d=> {
...
d3.select('#label_tag').text(data_select['Level']).attr('opacity',1)
    ...
    arc.append('text')
        ...
        .transition()
        .delay(function(d,i){return i*220})
        .attr('fill','black')
...
svg.selectAll('*').remove();
    ...
    var pie = d3.pie()  //🎒 explain: 
        .value(d => d.val_c)
        //.sort(null)  //🎒 explain: 
    .sort(function(a, b) { return a.Level.localeCompare(b.Level); })  //🎒 explain: 
    //.sortValues(d3.descending);  //🎒 explain: 
    var path = d3.arc()  //🎒 explain: 
        .outerRadius(radius)  //🎒 explain: 
        .innerRadius(100);  //🎒 explain: 
...
function updatev() {
    ...
    bottom5 = data.sort((a, b) => d3.ascending(a.full, b.full)).slice(0,13)
    }
    if(current_option == 'partial'){
        top5 = data.sort((a, b) => d3.descending(a.partial, b.partial)).slice(0,13)
    bottom5 = data.sort((a, b) => d3.ascending(a.partial, b.partial)).slice(0,13)
    }
filter(filterMode);
sort(sortMode);
redraw();
}
document.getElementById('close_type').addEventListener('change',updatev)
...
transition.selectAll('.name')
  ...
  .attr('x', d => x(d.Country) + x.bandwidth() / 2);
  x.domain(current.map(d => d.Country));
 transition = svg.transition()  //set-up the transition
        .duration(750)
        var xAxis;
xAxis = d3.axisBottom()
  .scale(x)
